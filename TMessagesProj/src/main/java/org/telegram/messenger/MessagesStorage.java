...
import org.telegram.tgcrm.features.deleted_messages.By;
import org.telegram.tgcrm.features.deleted_messages.DeletedMsg;
import org.telegram.tgcrm.features.deleted_messages.WhoDeletedMsg;
import org.telegram.tgcrm.features.save_edited_messages.EditMessage;
    ...
    public void deleteSavedDialog(long did) {
        storageQueue.postRunnable(() -> {
            ...
            try {
                ...
                cursor = null;
                if (!mids.isEmpty()) {
                    markMessagesAsDeletedInternal(selfId, mids, true, 0, 0, By.You);
    ...
    public void deleteUserChatHistory(long dialogId, long fromId) {
        storageQueue.postRunnable(() -> {
            ...
            try {
                ...
                markMessagesAsDeletedInternal(dialogId, mids, false, 0, 0, By.You);
...
//                        cursor = null;
            ...
            try {
                ...
                if (!dialogs.isEmpty()) {
                    for (int a = 0, N = dialogs.size(); a < N; a++) {
                        ...
                        markMessagesAsDeletedInternal(dialogId, mids, true, 0, 0, By.You);
    ...
    //    TgCrmDelete
    private ArrayList<Long> markMessagesAsDeletedInternal(long dialogId, ArrayList<Integer> messages, boolean deleteFiles, int mode, int threadMessageId, By whoDeleted) {
        SQLiteCursor cursor = null;
        SQLitePreparedStatement state = null;
        ...
        try {
            while (cursor.next()) {
                ...
                for (Integer message : messages) {
                    deletedMsgs.add(new WhoDeletedMsg(did, message, whoDeleted, deleteFiles));
                }
...
//                }
    ...
    public ArrayList<Long> markMessagesAsDeleted(long dialogId, ArrayList<Integer> messages, boolean useQueue, boolean deleteFiles, int mode, int topicId) {
        ...
        if (useQueue) {
            storageQueue.postRunnable(() -> markMessagesAsDeletedInternal(dialogId, messages, deleteFiles, mode, topicId, By.You));
        } else {
            return markMessagesAsDeletedInternal(dialogId, messages, deleteFiles, mode, topicId, By.You);
        }
        return null;
    }

    public ArrayList<Long> markMessagesAsDeleted(long dialogId, ArrayList<Integer> messages, boolean useQueue, boolean deleteFiles, int mode, int topicId, By whoDeleted) {
        if (messages.isEmpty()) {
            return null;
        }
        if (useQueue) {
            storageQueue.postRunnable(() -> markMessagesAsDeletedInternal(dialogId, messages, deleteFiles, mode, topicId, whoDeleted));
        } else {
            return markMessagesAsDeletedInternal(dialogId, messages, deleteFiles, mode, topicId, whoDeleted);
        }
        return null;
    }
