        ...
        switch (holder.getItemViewType()) {
            case VIEW_TYPE_DIALOG: {
                ...
                if (dialogsType == DialogsActivity.DIALOGS_TYPE_ADD_USERS_TO || dialogsType == DialogsActivity.DIALOGS_TYPE_BOT_REQUEST_PEER) {
                    ...
                    String ownerName;
                    if(TgCrmUtilities.getIfTransfer(dialog.id).isEmpty()){
                        ownerName=TgCrmUtilities.getOwnerName(dialog.id);
                    }else {
                        if (TgCrmUtilities.compareTime(dialog.id)) {
                            ownerName = TgCrmUtilities.getOwnerName(dialog.id);
                        } else {
                            ownerName = TgCrmUtilities.getIfTransfer(dialog.id);
                        }
                    }
                    cell.setDialog(dialog, dialogsType, folderId,ownerName);
                    cell.checkHeight();
